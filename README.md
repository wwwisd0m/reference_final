...
- `src/pages/game-play.css`, `BingoPage.tsx` — 빙고 시트/그리드. Excel 테마 빙고 **배치**는 A_G2_bingo_02 계열 토큰·행·열 슬라이드(`slideRowLabels` / `slideColLabels`)·FLIP. **플레이** 화면은 `bingo-play--g2-play` 등으로 Figma **A_G2_bingo_01**에 가깝게(페이지 배경 `#f9f9f9`, 흰 시트·그리드 `#e0e0e0`, 선택 대기 칸 `#e3f2fd`, 타이머 뱃지 `#e8f5e9` / `#2e7d32`).  
